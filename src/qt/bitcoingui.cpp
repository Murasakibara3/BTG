...
{
    resize(850, 550);
    setWindowTitle(tr("BITTOU COIN") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a BITTOU COIN address"));
...
//    connect(chatAction , SIGNAL(triggered()), this, SLOT(gotoOverviewPage()));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About BITTOU COIN"), this);
    aboutAction->setToolTip(tr("Show information about BITTOU COIN"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for BITTOU COIN"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("BITTOU COIN client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("BITTOU COIN client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to BITTOU COIN network.", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid BITTOU COIN address or malformed URI parameters."));
    }
